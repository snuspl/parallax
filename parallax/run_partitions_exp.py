...
def run_partitions_test(data_dir, apps, machine_nums, partitions, elsa_set):
  # create data_dir
  ...
  for app in apps:
    ...
    for num_machine in machine_nums:
      ...
      for partition in partitions:
        partition_dir = os.path.join(machine_dir, 'P%d' % partition)
...
if __name__ == '__main__':
  ...
  elsa_set = '3,1,11,6,7,8,10,2'
  elsa_set = ['elsa-' + '%02d'%int(elsa_idx) + '-ib0' for elsa_idx in elsa_set.split(',')]
  print(elsa_set)

  run_partitions_test(data_dir, apps, machine_nums, partitions, elsa_set)
