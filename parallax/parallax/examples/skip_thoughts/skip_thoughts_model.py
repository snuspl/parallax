...
class SkipThoughtsModel(object):
    ...
    def build_encoder(self):
        ...
        with tf.variable_scope("encoder") as scope:
            length = tf.to_int32(tf.reduce_sum(self.encode_mask, 1),
                                 name="length")

            if self.config.bidirectional_encoder:
                if self.config.encoder_dim % 2:
                    raise ValueError(
                        "encoder_dim must be even when using a bidirectional "
                        "encoder.")
    ...
    def _build_decoder(self, name, embeddings, targets, mask, initial_state,
        ...
        with tf.variable_scope(name) as scope:
            # Add a padding word at the start of each sentence (to correspond to
            # the prediction of the first word) and remove the last word.
