...
class ConvNetBuilder(object):
    ...
    def get_custom_getter(self):
        ...
        Currently, this custom getter only does anything if self.use_tf_layers
        is True. In that case, it causes variables to be stored as dtype
        self.variable_type, then casted to the requested dtype, instead of
        directly storing the variable as the requested dtype.
        """

        def inner_custom_getter(getter, *args, **kwargs):
            """Custom getter that forces variables to have type
            self.variable_type."""
            ...
            if not (requested_dtype == tf.float32 and
                ...
                # Only change the variable dtype if doing so does not decrease
                # variable precision.
                kwargs['dtype'] = self.variable_dtype
            var = getter(*args, **kwargs)
            # This if statement is needed to guard the cast, because batch norm
            # assigns directly to the return value of this custom getter. The
            # cast makes the return value not a variable so it cannot be
            # assigned. Batch norm variables are always in fp32 so this if
            # statement is never triggered for them.
    ...
    def get_variable(self, name, shape, dtype, cast_dtype, *args, **kwargs):
        # TODO(reedwm): Currently variables and gradients are transferred to
        # other devices and machines as type `dtype`, not `cast_dtype`.
        # In particular, this means in fp16 mode, variables are transferred as
        # fp32 values, not fp16 values, which uses extra bandwidth.
        ...
        with tf.variable_scope(name):
            ...
            if mode != 'SAME_RESNET':
                ...
                if d_height == 1 and d_width == 1:
                    ...
                    if self.data_format == 'NCHW':
                        padding = [padding[0], padding[3], padding[1],
                                   padding[2]]
    ...
    def _batch_norm_without_layers(self, input_layer, decay, use_scale,
        ...
        # using layers and not using layers.
