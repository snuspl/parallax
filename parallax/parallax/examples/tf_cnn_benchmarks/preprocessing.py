...
def eval_image(image,
    ...
    with tf.name_scope('eval_image'):
        ...
        # Compute resize_height and resize_width to be the minimum values
        # such that
        #   1. The aspect ratio is maintained (i.e. resize_height / resize_width
        #       is image_height / image_width), and
...
class Cifar10ImagePreprocessor(object):
    ...
    def _eval_image(self, image):
        ...
        if self.summary_verbosity >= 3:
            tf.summary.image('cropped.image',
                             tf.expand_dims(distorted_image, 0))
        return distorted_image
    ...
    def minibatch(self, dataset, subset, use_datasets, cache_data,
        ...
        with tf.name_scope('batch_processing'):
            ...
            # batch. Without the unstack call, raw_images[i] would still access
            # the same image via a strided_slice op, but would be slower.
            raw_images = tf.unstack(raw_images, axis=0)
            raw_labels = tf.unstack(raw_labels, axis=0)
            for i in xrange(self.batch_size):
                split_index = i % self.num_splits
                # The raw image read from data has the format [depth, height,
                # width] reshape to the format returned by minibatch.
                raw_image = tf.reshape(raw_images[i],
                                       [dataset.depth, dataset.height,
                                        dataset.width])
